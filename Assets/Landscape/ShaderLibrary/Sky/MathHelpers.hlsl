#ifndef SKY_MATH_HELPERS_INCLUDE
#define SKY_MATH_HELPERS_INCLUDE
...
static const float3 RANDOM_SPHERE_SAMPLES[64] =
{
...
float Lerp01(float x, float xStart, float xEnd)
{
    float xNorm = (x - xStart) / (xEnd - xStart);
    float y = lerp(0, 1, xNorm);
    return saturate(y);
}
...
float2 RayHitSphere(Ray ray, float3 center, float radius)
{
    float3 co = ray.origin - center;
    float a = dot(ray.origin, ray.origin);
    ...
    return float2(-b + dSqrt, -b - dSqrt) / ( 2.0 * a);
}

float RayHitSphereClosest(Ray ray, float3 center, float radius)
{
    float os = length(center - ray.origin);
    float sh = dot(center - ray.origin, ray.direction);
    ...
    // if t1 and t2 are both positive or negative, use closet distance, otherwise use distance which is positive
    float t1 = sh - ph;
    float t2 = sh + ph;
    float t = (t1 < 0) ? t2 : t1;
...
float CompressLatitude(float latitude)
{
	return 0.5 + 0.5 * sign(latitude) * sqrt(abs(latitude) * 2.0 / PI);
}

float DecompressLatitude(float compressedLatitude)
{
    float factor = 2.0 * compressedLatitude - 1.0;
    return sign(factor) * 0.5 * PI * factor * factor;
}
